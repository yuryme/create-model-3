...
- Удаление старых `catalog/recipe` и `register/recipe_component` после периода наблюдения за рецептами — отдельное решение PM.
...
- `sp-003-recipe-doc` полностью завершён (PM 2026-06-13): plan approved, metadata реализована (`records/recipe_norm`, `operation/recipe_doc` с forms/command, `workflow/migrate_recipe_docs`, перевод `requirement_calc` и `production_output.calc_raw_writeoff` на `records.recipe_norm`), импорт и functional acceptance прошли успешно, все артефакты переведены в `implemented`, metadata и root package закоммичены. Старые `catalog/recipe` и `register/recipe_component` сохранены как архив; их удаление — отдельное решение PM.
- Update блока рецептов — scope-решения PM 2026-06-12: старый рабочий UX `catalog/recipe` + `register/recipe_component` заменяется на документ `operation/recipe_doc` («Рецепт») с табличной частью компонентов и проведением в `records/recipe_norm`; в строках документа количество компонента задаётся на `output_qty` из шапки, а нормализация на 1 единицу выхода хранится в `recipe_norm`; `is_active` в шапке не создаётся; актуальность = последний проведённый рецепт по `output_item` с `period <= дата расчёта`; два рецепта на один выход с одной датой запрещены; старые рецепты проигрывают по дате, но остаются историей; старые данные мигрируются автоматически; единицы показываются справочно, пересчёта единиц нет; выход только `product`/`semi_product`, компонент только `raw`/`semi_product`; запрет компонента с единицей `шт` снят; формы list/edit обязательны.
- `sp-003-recipe-doc` review-пакет approved PM 2026-06-12: `project/docs/specs/sp-003-recipe-doc-design.md`, `project/docs/specs/sp-003-recipe-doc.json`, `project/docs/specs/sp-003-recipe-doc.architecture-view.json`, `project/docs/specs/sp-003-recipe-doc.metadata-view.json`, `project/docs/specs/sp-003-recipe-doc.architecture-view.html`; spec-json прошёл `python project/docs/specs/validate_spec.py project/docs/specs/sp-003-recipe-doc.json`. PM-review замечания C1/C2/C3 закрыты правками: дата миграции для исторических пересчётов, guard от деления на ноль, явная ветка `production_task.calc_requirement -> requirement_calc`; следующий gate — metadata-реализация.
