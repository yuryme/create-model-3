...
**Последнее обновление:** 2026-06-02 — metadata части 1a реализованы и импортированы в BaSYS без ошибок (17 metaobject, локальный commit `metadata/` `bd7bbe9`); план + import-notes запушены в workspace-репо. ТЗ/план — `approved` до функциональной приёмки (чек-лист C). Поставлена новая задача sp-002 — генератор тестовых данных (сеялка) для цепочки 1a; поручение на методику/ТЗ передаётся аналитику.
...
1. Функциональная приёмка 1a на стенде (чек-лист C); после неё — `implemented` для ТЗ и плана.
2. sp-002 — сеялка тестовых данных: аналитик пишет методику `sp-002-seed-design.md`, затем ТЗ. Ключевой вопрос методики — bootstrap операций (`create-fill-workflow` покрывает только enum/catalog/register).
