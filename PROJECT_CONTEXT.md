...
**Последнее обновление:** 2026-06-02 — методика sp-002 (загрузка тестовых данных 1a) переведена в `approved` после трёх кругов ревью; следующее — ТЗ `sp-002-seed-data-1a.md`. Ранее: metadata 1a реализованы и импортированы без ошибок (commit `metadata/` `bd7bbe9`), ТЗ/план 1a — `approved` до функциональной приёмки. Введено правило доказательности (Evidence Rule в AGENTS.md, ADR-2026-06-02).
...
1. sp-002 — аналитик пишет ТЗ `sp-002-seed-data-1a.md` (два workflow, поля маппинга, тестовый набор, чек-лист); затем PM-chat review → engineer plan.
