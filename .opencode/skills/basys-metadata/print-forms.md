...
A **print form** in BaSYS is a per-instance document rendered for a specific metaobject record (by its primary key) by filling a pre-designed `.xlsx` template with values from data sources. The same template engine as for [Excel Reports](../basys-metadata/excel-reports.md) — `ClosedXML.Report` — is used; the difference is the binding and the trigger:
...
| Self-standing report over a selection (regulated form, summary)       | [Excel Report](../basys-metadata/excel-reports.md) |
| Interactive table / chart / dashboard / KPI                           | [Data View](../basys-metadata/data-view-reports.md) |
...
Same as Excel reports — see the [excel-reports rule](../basys-metadata/excel-reports.md) for the full table. Quick recap:
